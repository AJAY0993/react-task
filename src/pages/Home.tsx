...
function Home() {
    ...
    const sort = (sortBy: string) => {
        ...
        if (sortBy === "id") {
            setVisibleUsers([...visibleUsers].sort((a, b) => {
                if (a.id && b.id) {
                    return a.id - b.id
                }
                return 0
    ...
    return (
        <div>
            ...
            <div className='flex justify-between gap-2 p-2'>
                <input className='input' placeholder='Search' onChange={(e) => handleChange(e.target.value)} />
                <select className='bg-black text-white rounded focus:outline-none p-2 cursor-pointer' defaultValue="id" onChange={(e) => sort(e.target.value)}>
            ...
            <header>
                <div className='w-full flex bg-[#2C3E50]  p-2 ' role='row'>
                    <div className='min-w-6  text-white border-r-[1px] text-center' role='column'>Id</div>
                    <div className='flex-1 text-white border-r-[1px] text-center' role='column'>Name</div>
                    <div className='flex-1 text-white text-center hidden md:block' role='column'>Email</div>
                    <div className='text-white text-center ml-4 md:ml-0' role='column'><GoGear /></div>
                </div>
            </header>

            {/* User list or skeleton loading */}
            <ul role="table">
                {isLoading && [1, 1, 1, 1, 1, 1, 1].map(() => <Skeleton />)} {/* Show skeleton loading when data is being fetched */}
                {users && visibleUsers.length > 0 && visibleUsers.map((user, i) => <UserRow user={user} key={user.id} setUsers={setUsers} index={i} />)}
            </ul>

            {/* Modal for creating new user */}
            <ModalProvider.Open id={'create'}>
                <button className='w-[90%] mx-auto block mt-4 rounded p-2 min-w-24 text-center bg-blue-400 text-white'>Create</button>
            </ModalProvider.Open>
