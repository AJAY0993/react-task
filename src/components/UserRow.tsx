...
function UserRow({ user, setUsers, index }: { user: User, setUsers: Dispatch<SetStateAction<User[]>>, index: number }) {
    ...
    const deleteRow = async () => {
        if (user.id) {
            ...
            setUsers(users => users?.filter(currentUser => currentUser.id !== user.id)) // Update the user list
        }
    }
    ...
    return (
        <li className='w-full flex p-2' role='row' style={{ backgroundColor: bg }}>
            {/* User ID */}
            <div className='min-w-6 text-white border-r-[1px] text-center' role='column'>{user.id}</div>

            {/* User name with a link to their profile */}
            <div className='flex-1 text-white border-r-[1px] text-center' role='column'>
                <Link className='underline' to={user.id?.toString() || ""} state={{ user }}>
                    {user.name}
                </Link>
            </div>

            {/* User email, hidden on small screens */}
            <div className='flex-1 text-white text-center hidden md:block' role='column'>{user.email}</div>
            ...
            <div className='text-white text-center' role='column'>
                <div className='flex items-center gap-2 ml-4 md:ml-0'>
                    ...
                    <ModalProvider.Window id={'delete' + user.id}>
                        <div className='bg-white rounded-md p-4'>
                            ...
                            <div className='flex gap-2 w-fit ml-auto'>
                                <button className='rounded p-2 min-w-24 text-center bg-black text-white' onClick={closeModal}>Cancel</button>
                                <button className='rounded p-2 min-w-24 text-center bg-red-600 text-white' onClick={deleteRow}>Delete</button>
                            </div>
                        </div>
                    ...
                    <ModalProvider.Window id={'update' + user.id}>
                        <Form mode='update' user={user} setUsers={setUsers} /> {/* Pass user and setUsers to form for update */}
                    </ModalProvider.Window>
                </div>
            </div>
        </li>
