...
function OverLay({ children }: { children: ReactNode }) {
  ...
  return createPortal(
    <div className="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-50">
      <button
        className="fixed top-4 right-4 text-white bg-transparent rounded p-2 hover:bg-neutral-3 dark:hover:bg-dark-blue-2"
        onClick={close}
      >
...
export function useModalContext() {
  ...
  if (context === undefined)
    throw new Error("useModalContext was called outside of ModalProvider"); // Ensure context is available
  return context;
}
