...
function App() {
  return (
    <BrowserRouter>
      ...
      <ModalProvider>
        {" "}
        {/* Wrapping components with the ModalProvider to use modals across the app */}
        <Routes>
          {" "}
          {/* Defining a set of routes for the app */}
          <Route path="/" index element={<Home />} />{" "}
          {/* Defining the route for the home page */}
        </Routes>
        <Routes>
          {" "}
          {/* Another set of routes for dynamic user pages */}
          <Route path="/:id" element={<User />} />{" "}
          {/* Defining a dynamic route that captures an 'id' parameter */}
        </Routes>
      </ModalProvider>
      <Toaster position="top-center" />{" "}
      {/* Adding a toast notification with 'top-center' positioning */}
    </BrowserRouter>
  );
}
